#!/usr/bin/env python3
"""Sistema de autenticação com registro de usuá        except Exception as e:
        print(f"❌ Erro ao configurar plano empresarial: {e}")

def ensure_empresarial_user():reamlit as st
import re
import json
import os
...
def setup_empresarial_user():
    """DEPRECIADA: Use ensure_empresarial_user() em vez desta"""
    pass  # Função mantida para compatibilidade mas não faz nada
...
def empresarial_login():
    """Login automático no modo empresarial"""
    email, password = setup_empresarial_user()
    
    # Faz login automático
    success, user = db.authenticate_user(email, password)
    if success and user:
        st.session_state.authenticated = True
        st.session_state.current_user = user
        st.session_state.user_id = user['id']
        st.session_state.user_email = user['email']
        st.session_state.username = user['username']  # Adicionando username que estava faltando
