...
from utils import display_message_with_spoiler, dict_to_markdown_table
...
with st.expander("⚠️ Limitações e Regras do Assistente (clique para ver)", expanded=False):
    st.markdown(
        ...
        - As consultas e agrupamentos **são limitados às dimensões**: ano, mês, UF, modelo e loja.
...
# Captura novo input
prompt = st.chat_input("Pergunte sobre as vendas de veículos...")
...
if prompt:
    ...
    reuse_keywords = ["usando os mesmos dados", "com os mesmos dados", "nos dados anteriores"]
    ...
    try:
        if should_reuse and st.session_state.last_data["raw_data"]:
            ...
            processing_msg.chat_message("assistant").markdown("Processando sua solicitação...")
            ...
            if response.candidates and response.candidates[0].content.parts[0].function_call:
                ...
                if "error" in raw_data:
                    ...
                    processing_msg.chat_message("assistant").markdown("Dados recebidos. Calculando resultados...")
